...
float cpu_unloaded = 0.0;
float cpu_loaded = 0.0;
...
static volatile char frequency[50];
...
void waveform_func(UArg arg1, UArg arg2)
{
    ...
    while (true) {
        ...
        else
        {
            ...
            for(j=0;j<NFFT;j++)
            {
                fft_sample[j] = gADCBuffer[ADC_BUFFER_WRAP(getADCBufferIndex() - j)];
            }
        }
...
void display_func(UArg arg1, UArg arg2)
{
    ...
    while (true) {
        ...
        if (!fft_mode){
            ...
            //Draw frequency measurement
            GrStringDraw(&sContext, frequency, -1,0,110,false);
...
void freq_func(UArg arg1, UArg arg2)
{
    float signal_f = 0.0;
    float total_interval_sec;
    while (true)
    {
        Semaphore_pend(freq_sem, BIOS_WAIT_FOREVER);

        //Using GateHwi to protect access to shared data
        IArg key_freq = GateHwi_enter(gateHwi1);

        //calculate the frequency
        total_interval_sec = ((float) acc_interval) / 120000000;
        signal_f = ((float) total_period) /total_interval_sec;

        //reset globals for new measurements
        acc_interval = 0;
        total_period = 0;

        GateHwi_leave(gateHwi1, key_freq);

        //write frequency into an array for display
        snprintf(frequency, sizeof(frequency), "f = %.3f Hz", signal_f);
    }
}
